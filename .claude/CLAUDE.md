...
- **An embed page is the ONE frameable path** (`lib/embeds.ts`): Slack iframes a `video` block, so `/embeds/` is served without `X-Frame-Options`. Everything else — the dashboard above all — keeps it. Only `kyto.dino.icu` is a registered unfurl domain, so an arbitrary site can never be embedded.
...
- **The plan-card budget is PER PLAN MESSAGE, not per turn** (`lib/ai/stream/cards.ts`, tested). A turn is rendered across several plan messages (a segment split, a 4.5-min rotation) and a card id only exists inside the `chatStream` it was appended to. When the budget lasted a whole attempt, every message after the first showed only "N more tool calls" — a month of that. The CALLER owns it and calls `endMessage()` at each boundary, which also completes anything mid-flight.
