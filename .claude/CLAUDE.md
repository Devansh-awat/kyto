...
> **Check `TODO.md` when touching related files.** Before finishing a change,
> skim `TODO.md` — if an open item lives in the same area you're editing, tell
> the user and ask whether to fold it into this change rather than leaving it to
> rot. Remove items from `TODO.md` in the same commit that resolves them.
...
`codeMode` (core, `tools/code-mode.ts`) is the Cloudflare [Code Mode](https://developers.cloudflare.com/agents/tools/codemode/) pattern: instead of one host tool per step (each a full model round-trip that can 429 or exhaust the step budget mid-way), the model writes **ONE TypeScript program** that does the whole multi-step job and prints its result. Runs in the thread's persistent sandbox via `bun` (native TS). The script can `import { sh, slack } from './kyto.ts'` (an always-refreshed helper: shell + the READ-ONLY Slack proxy), use `fetch`, and drive `cloakbrowser`. `install` pre-`bun add`s deps. This is the answer to the "50 browser round-trips" failure — script the loop, don't click 50 times.
...
kyto genuinely SEES images now — for a long time it couldn't, which is why it failed anything visual (e.g. reading a slider captcha). Attachments were only downloaded into the sandbox and handed over as file PATHS (`promptWithAttachments`); no image ever reached the model.
- **The only channel that works is a USER message.** The openai-compatible providers (how K2.6/DO/HackClub are reached) **JSON-stringify tool-result content**, so an image returned from a tool is dropped to text and never seen. Verified: a tool-result image is lost; a user-message image part converts to `image_url` and IS seen. So both vision paths land as user-message file parts.
- **Attachment images** (`seedAttachments` keeps `imageBytes` for png/jpg/webp/gif ≤8MB) are passed to `streamAttempt` as `images` and ride in the user turn (before the text, so the cache breakpoint stays on the trailing text).
- **`viewImage`** (core tool, `tools/view-image.ts`) lets the model look at a sandbox image (a screenshot it took, a generated/downloaded image). It pushes the bytes into a per-turn buffer (`buildTools` → `drainImages`); `streamAttempt`'s `getFreshImages` is drained in `prepareStep`, which appends the image as a **user message** for the next step (the override carries forward, so it stays visible). `readFile` still returns text — the sandbox prompt tells the model to `viewImage` instead of assuming it's blind.
- The **primary Kimi K2.6 has the MoonViT vision encoder**, so no vision-specific model routing is needed — images go to whatever model runs. Both the main loop and the subagent get `getFreshImages`.

### Memory (workspace-global)

`saveMemory`/`fetchMemory`/`editMemory` (core, `tools/memory.ts`; `memories` table; `@repo/db` `listMemoryIndex`/`getMemory`/`createMemory`/`updateMemory`). Durable notes kyto writes for ITSELF after solving a big/non-obvious task, so a LATER thread reuses the solution instead of re-deriving it. **Workspace-global, not per-user** — shared across everyone. Every memory's `title` + one-line `summary` is injected into the system prompt as a `<memories>` block (`RequestHints.memories`, filled by `requestHints` via `listMemoryIndex`, rendered in `prompts/context.ts`); the model calls `fetchMemory("<title>")` to pull the full `body` only when a listed memory is relevant. `saveMemory` is create-only (title is unique; a clash tells the model to edit); `editMemory` patches summary/body in place. **There is deliberately NO delete tool** — the knowledge base can only grow. Titles are the handle; keep them stable.
...
- **`bash` auto-backgrounds a slow command** (`tools/sandbox.ts` + `tools/background.ts`): a foreground `bash` command still running after **60s** (`AUTO_BACKGROUND_MS`) is moved to the background instead of freezing the turn — the tool hands back a handle (`bg-N`) with `running: true` and the model polls it via `getProcessOutput`/`killProcess`. Implemented by running EVERY `bash` command detached (nohup, separate stdout/stderr/exit files) and polling for up to 60s; a fast command returns transparently, a slow one returns the handle. The `bash` tool shares the **one** `backgroundProcessTools` registry (built before `core` in `toolset.ts`), so `runBackgroundProcess`/`getProcessOutput`/`killProcess` and the auto-moved jobs all share ids. This is what stops a benchmark/runaway script from hanging a turn on an empty message (the old failure). Handles are per-turn (in-memory), same as the rest.
...
There is **no search method in the allowlist**, so "count a user's messages" means paging `conversations.history` per channel — slow, not a bug. Add `search.messages` (needs `search:read`) if that's ever wanted. NOTE: a subagent **shares the parent turn's `LazySandbox`** (`getSandboxContext`), so `slackScript`/`codeMode`/the `slack` PATH helper work inside a subagent too — `run()` sends the parent turn's proxy env on every command, and that token stays valid for the subagent's lifetime (it runs within the parent turn). (Historically the subagent got its own env-less sandbox and 401'd; that's no longer the case.)
...
- **Broadcast mentions are owner-gated AND channel-local.** Only the owner may make kyto ping a whole channel, and only in the channel kyto was invoked in. `neutralizeBroadcast` (`harness/markdown.ts`) downgrades `<!channel>`/`<!here>`/`<!everyone>`/`<!subteam^…>` to inert plaintext; it's applied to the streamed reply (`createReply({allowBroadcast})`, `allowBroadcast = isOwner`) and, in `postMessage`, whenever the target isn't the current channel — **owner included** (`allowBroadcast = isOwner && target === currentChannel`). Being allowed to post into #announcements is not the same as being allowed to @channel a room kyto isn't part of the conversation in; the owner can send that ping from the channel itself. `neutralizeBroadcastDeep` applies the same strip to every string in a Block Kit payload, since a block's `text` notifies exactly like message text.
- **`postMessage` can send Block Kit**: an optional `blocks` param (JSON array string, ≤50 blocks, validated by `parseBlocks`). It replaces the markdown body; the `message` param stays required and becomes the notification fallback text (`PostContent.fallbackText`).
- **`postMessage` identity override is OWNER-ONLY** (`lib/post-identity.ts`): `asName`+`asIcon` post under a fully custom name/avatar, or `asUser` (a user/bot id or @mention) mirrors that person/bot's display name + avatar (`users.info`/`bots.info`). A non-owner (who can only post same-channel) can't use it — wearing another member's name is the impersonation vector. The chosen identity rides through the confirm-post gate (`PendingPost.identity`) so a cross-channel send wears the face the owner approved. Slack still tags a customized bot post as an app, so it dresses a message up rather than truly taking over an account. Applied in `executePostMessage` (override replaces the configured `resolveIdentity('normal')`).
...
- **Outward-facing posts need a human confirm click** (`lib/confirm-post/`, `features/confirm-post/`): a cross-channel/DM `postMessage`, and EVERY `sendAsUser`/`editAsUser`, no longer fire inline. The tool stashes the pending post (`stashPendingPost`, in-memory, 10-min TTL, single-use) and shows the **owner** an ephemeral Block Kit **Confirm & send / Cancel** button in the current thread; the send only happens in the `confirm_post_send` action handler, which **re-checks the clicker is `OWNER_USER_ID`**. This is the anti-injection gate: a prompt injection can *request* an outward post but cannot press the button. The actual send is the shared `executePostMessage`/`executeSendAsUser`/`executeEditAsUser` (same code as an inline post), so Block Kit payloads survive the round-trip. The ephemeral is replaced with the result via the interaction `response_url` (`respondToInteraction`). Same-channel replies still post immediately — no confirmation. Non-owners are still refused outright, before any confirmation.
...
- **`##` messages are invisible to kyto.** A message that **starts with** `##` (after stripping leading mentions) is a human-only side-channel: `isHiddenFromBot` makes `shouldIgnore` skip it AND `buildPrompt` filter it out of replayed history — so kyto never triggers on it and never sees it. Only the FIRST content line counts — a `##` later in the message (e.g. an ordinary message containing a markdown `## heading`) does NOT hide it. It used to trigger on ANY line, which silently dropped normal messages.
...
- **Primary is pinned `moonshotai/kimi-k2.6` on HackClub** (`PRIMARY_ATTEMPT`, `packages/ai/src/providers/attempts.ts`) — owner's call, replacing the DigitalOcean-BYOK primary (DO was rate-limiting the whole roster). So **every turn now spends HackClub's daily $3 cap**; DigitalOcean is the first fallback tier (free) and a budget/outage failure short-circuits to it. The HackClub outage counter only counts failures the proxy reported (an HTTP status), or one bad completion from the primary would write off the whole HackClub leaderboard for the turn.
- **`LEADERBOARD_FALLBACK` is EXACTLY the owner's arena top 19** (one rung per model, at its best rank — the arena ranks reasoning-effort variants separately but the proxy has one slug each). When the owner hands over a new leaderboard, **replace** the list; never merge the old tail back in — that tail is where the junk accumulates.
- **An ATTEMPT is "handled" iff IT produced reply text or a deliberate `skip`** (per-attempt, not per-turn — a continuation attempt inherits the turn's text and would otherwise report success having said nothing). A model that ran tools but wrote nothing gets ONE `synthesizeFinalAnswer` nudge (same model, `tools: {}`) before falling back.
- **Fallback walks by TIER, best-first within each** (`buildFallbackQueue`): the rest of the DigitalOcean BYOK roster (free, owner's quota), then the HackClub leaderboard in rank order starting at opus-4.8, then the owner's Gemini key. It must NOT pivot on the primary's rank — the pinned primary sits at the BOTTOM of `LEADERBOARD_FALLBACK`, so the old "walk up from the pivot" reversed the leaderboard and fell back worst-first, landing on nemotron. **Every rung must be a model good enough to hand a live thread to**; reachable on the proxy is not the bar.
- **A model that stops answering and starts LOOPING is not "handled"** (`lib/agent/degenerate.ts`, `DegenerateOutputError`): nemotron once streamed "@devansh" several hundred times into a public thread, and it counted as a successful turn because a loop is still, formally, reply text. A repetition guard trips on 8 identical consecutive lines (outside a code fence) or a runaway single line, keeps the loop out of Slack (`reply.drop()`), scrubs it from the next model's continuation context, and falls back.
- **A rate-limited tier is written off for the turn**: DigitalOcean's BYOK limit is per ACCOUNT, so the first 429 skips the whole DO roster (`digitaloceanRateLimited`) instead of walking nine doomed rungs — same idea as the HackClub budget/outage short-circuits.
...
- **Memory = the Slack thread.** `buildPrompt` feeds the whole thread (`slack.fetchMessages`, capped) as context; no verbatim message TRANSCRIPT is persisted — the sandbox persists a *filesystem*, not a chat log, and `langfuse` tracing stays disabled. Note: kyto DOES now persist two kinds of DERIVED text with a ~30-day retention — the `thread_thinking` reasoning cache and workspace `memories` (both can paraphrase message content). The owner signed off on this retention window explicitly; it is a deliberate shift from the earlier "store nothing" stance, not an oversight to revert.
- **…plus the last few turns' THINKING** (`lib/agent/thinking.ts`). Slack only records what kyto *said*; the reasoning behind it lived in the plan's Thinking cards and was then gone, so every turn re-derived the previous turn's conclusions and dead ends. `renderStream`'s `onReasoning` collects each turn's reasoning; `rememberThinking` keeps the last 3 turns per thread (tail-clamped) and `buildPrompt` injects them as `<your_previous_thinking>`. **Persisted to Postgres** (`thread_thinking` table, ~30-day retention via `RETENTION_MS`, daily `startThinkingReaper`) so it survives a restart — it used to be in-memory and was wiped on every deploy, which is why a thread picked back up after a restart lost its train of thought. `rememberThinking`/`recallThinking` are async and best-effort. Only the attempt that ANSWERED leaves its thinking behind; a failed or degenerate attempt's reasoning dies with it, so a spiral can't seed the next turn.
