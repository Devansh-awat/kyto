...
- **A subagent runs on the SAME model as the turn** (`subagentAttempts`, deepseek-v4-flash first, Gemini behind it — owner's call 2026-08-21). **Compaction did NOT follow it**: `compactionAttempts` stays Gemini-first, because one pass digests up to 200 messages, a long-idle thread catches up over many passes, and it all runs in the background where nobody sees the bill — on the shared $3/day cap a single 25k-message backlog could spend the day on bookkeeping.
