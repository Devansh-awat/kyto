...
- **"no tools loaded" is never a reply either** (`lib/ai/stream/tool-complaints.ts`, tested; owner's calls 2026-08-22 "FIX ONCE AND FOR ALL" + "NEVER LAUNCH MODELS WITHOUT tools"). Two layers, in this order: (1) the prose-only recovery calls (truncated-reply continuation, subagent report nudge, reminder report nudge) carry their REAL toolset now, and their prompts ask for prose without naming tools at all — an empty `tools: {}` against a system prompt describing fifty of them is what made weak models narrate "no tools loaded". The one deliberate no-tools call left is compaction, whose standalone summarizer prompt never mentions a toolset. (2) ON TOP of that, such a sentence is dropped where reply text is emitted — but ONLY where the caller declares the call prose-only (`dropToolComplaints` / `stripToolComplaints`), so a normal turn filters nothing and "which tools do you have?" still answers honestly. Layer 2 exists because three rounds of wording fixes kept regressing into whichever call site went hot next; do not replace it with another prompt tweak. Mechanics and the regression history in [`.claude/STREAMING.md`](./STREAMING.md).
