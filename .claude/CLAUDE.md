...
- **Primary is pinned `minimax-m2.5` on DigitalOcean BYOK** (`PRIMARY_ATTEMPT`, `packages/ai/src/providers/attempts.ts`) — the main query is billed to the owner's DO quota, so HackClub's daily $3 cap is only spent on fallback. Degrades to HackClub `z-ai/glm-5.2` if `OPENROUTER_API_KEY` is unset.
