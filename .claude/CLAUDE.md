...
- **…plus the last few turns' THINKING** (`lib/agent/thinking.ts`). Slack only records what kyto *said*; the reasoning behind it lived in the plan's Thinking cards and was then gone, so every turn re-derived the previous turn's conclusions and dead ends. `renderStream`'s `onReasoning` collects each turn's reasoning; `rememberThinking` keeps the last 3 turns per thread (tail-clamped, 12h TTL) and `buildPrompt` injects them as `<your_previous_thinking>`. **In memory only — never a DB table**: reasoning quotes message contents freely, and persisting it would break the no-stored-contents position above. Only the attempt that ANSWERED leaves its thinking behind; a failed or degenerate attempt's reasoning dies with it, so a spiral can't seed the next turn.
