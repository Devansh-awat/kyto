...
- **Per-user MCP servers** (`lib/ai/mcp.ts`, `user_mcp_servers`): remote Streamable-HTTP servers added from **App Home**. A hand-rolled JSON-RPC client connects lazily per turn; listings cached 10 min; tools namespaced `mcp_<server>_<tool>`, deferred behind `loadTools`. A dead server degrades only that turn.
  - **A bare token is normalized to `Bearer <token>` at save time** (`normalizeMcpAuthorization`, tested), and a **failed listing is RECORDED, not just logged** (`getMcpFailure`, shown on the entry in App Home). Together they were one bug: the field wants a header VALUE, but an API token is what providers hand you and so what gets pasted, and with no scheme every bearer-auth server 401s — which the old catch swallowed, making a bad entry look exactly like a server with no tools. A valid Coolify token sat dead for a day. The record doubles as a 60s negative cache: `buildMcpTools` is awaited while the toolset is built, so a broken entry otherwise added up to two 8s timeouts to every one of that user's turns. The listing cache is keyed by URL **and** credential — on URL alone it served one user's listing to another.
...
- **An embed page is the ONE frameable path** (`lib/embeds.ts`): Slack iframes a `video` block, so `/embeds/` is served without `X-Frame-Options`. Everything else — the dashboard above all — keeps it. Only `kyto.dino.icu` is a registered unfurl domain, so an arbitrary site can never be embedded. A **whiteboard** embed is multiplayer (`lib/whiteboard/`, **Excalidraw**, our own four-message sync over a WebSocket at `/whiteboard/<id>`); a socket is refused unless kyto PUBLISHED that board, so nobody can mint documents on kyto's host, and the client is built by kyto rather than pulled from a CDN. **tldraw is banned** — its licence forbids production use without a paid key and 5.x blanks the editor 5s after load to enforce it. Detail in TOOLS.md.
...
- **`/kyto ban @someone 1d <reason>` is OWNER-ONLY and stops kyto answering them** (`lib/bans/`, `banned_users`, owner's ask 2026-08-14). Also `/kyto unban @someone` and `/kyto bans`; the same three work as `@kyto!ban …`, and BOTH go through `runBanCommand`, which does the owner check itself so neither entry point can forget it. **The model has no ban tool** — an injection cannot reach it. A ban gates being ANSWERED, not being read: their messages stay in the thread history kyto reads, because a conversation with holes in it makes no sense. A banned person gets ONE ephemeral (never a public reply, never a DM) naming the reason and the expiry. Duration parsing is the one part with a wrong answer available, so it is split out and tested (`bans/duration.ts`): `30m`/`2h30m`/`1d`/`1w`/`forever`, capped at a year, and anything that is not a duration is REFUSED rather than defaulted — `1dave` must not become a one-day ban with the reason eaten. A reason is required. Expiry is `expiresAt > now` at read time, so a ban lifts itself with no reaper.
...
- **A provider whose terms allow training on what is sent may NOT be a tier, at any position** (owner's call 2026-08-11). OpenCode Zen's free models were wired and are now removed for exactly this: Hack Club's scraping policy permits training on Slack messages only with every author's explicit consent, and a turn carries other people's messages. `buildFallbackQueue` is an ALLOWLIST of tiers so an unchosen provider is dropped rather than tried last (tested). Read a candidate's terms before adding it.
