...
- **Broadcast pings are DENIED BY DEFAULT in `ThreadHandle.post`** (`PostContent.allowBroadcast`, off unless set). It used to be opt-IN, and the paths that forgot were the ones nobody thinks of as "the model talking": REMINDERS (model-authored, an agent reminder's whole body, creatable by anyone) and the `title` on `mermaid`/`uploadFile` — and `post` renders a control mention as a `section`+`mrkdwn` block precisely so it resolves into a real ping, so those pinged whole channels ungated. Only two callers opt in: the owner's streamed reply, and an owner's SAME-CHANNEL `postMessage`. Omitting the flag fails CLOSED. The strip is field-by-field (markdown/fallbackText/blocks), not a deep walk — that would turn `files[].data` from a Uint8Array into a plain object.
- **The approval gate is persisted, public, and never expires** (`approval_requests`, `lib/approvals/`, `features/approvals/`). A non-owner's cross-CHANNEL post, a broadcast the gate would otherwise strip, and a third-party GitHub write are queued rather than refused; the turn does NOT block on one (a request can sit for hours — holding the loop open would burn the watchdog and strand the rest of the message). Load-bearing: only `OWNER_USER_ID` may decide (buttons are PUBLIC, so without that check the asker could approve themselves); the action runs from the row written when the request was MADE, never from the click, so a later injection in the same thread can't redirect an approved post; `kind` is a CLOSED set re-validated at execute time; the claim is `status = 'pending'` in the UPDATE, so a double-click can't send twice; approving a GitHub request grants trust and does NOT re-run the command. **`sendAsUser`/`editAsUser` are deliberately NOT an approval kind and must never become one** — posting as the owner keeps its synchronous confirm click and has no approval path at all.
...
- **Outward-facing posts need a human confirm click** (`lib/confirm-post/`, `features/confirm-post/`): a cross-channel/DM `postMessage` and EVERY `sendAsUser`/`editAsUser` stash the pending post (`stashPendingPost`, 10-min TTL, single-use) and show the **owner** a **Confirm & send / Cancel** in-thread (falling back to the owner's DM, summary naming the requester). The send fires only in `confirm_post_send`, which **re-checks the clicker is `OWNER_USER_ID`** — an injection can *request* an outward post but can't press the button. Same-channel replies post immediately.
...
- **Kyto is closed-source (for now).** `prompts/slack.ts` says kyto's code is private with no public repo — do NOT point users at `imdevarsh/gorkie-slack` as "kyto's source". A **source-available `LICENSE` exists** (all rights reserved; gorkie-derived code stays MIT in `LICENSE-gorkie-MIT`) and the pre-publication audit is DONE — every licence question resolved, only the `prompts/slack.ts` line left to flip in the same commit as going public. Until then the repo is PRIVATE; leave that line alone. Detail: `docs/reference/publishing.md`.
- **Owner grounding**: `RequestHints.ownerUserId` (from `OWNER_USER_ID`) renders into the context block as a plain statement of who owns/built kyto, with an instruction not to hedge or invent a different origin (without it, kyto confabulated an origin and disputed the real owner's correction).

### Identity profiles

`identity_profiles` table (`message_type` PK, `icon`; live types `normal`|`reminder`), owner-configured from **App Home "Identity"**. **Icon only — name suffixes removed** (owner's call, 2026-07-26, `name_suffix` column dropped): kyto's display name is only ever "kyto", a subagent card only ever "kyto subagent"/"kyto subagent {name}" (fixed in `subagent.ts`; the `subagent` profile type is gone). `resolveIdentity(type)` (`lib/identity.ts`, 30s cache) returns icon fields only (`icon` = `:emoji:` or image URL; `ResolvedIdentity.username` exists solely for `lib/post-identity.ts` overrides). `normal` applies to streamed replies AND cross-channel `postMessage`; `reminder` to reminder posts. Needs `chat:write.customize`.
...
- **Reasoning** renders under `Thinking` with a **UNIQUE task id per block**: providers reuse the same `part.id` for every step's reasoning, which used to collapse all thinking into one pinned row. `renderStream` mints `reasoning-${counter++}` per `reasoning-start`, so the plan reads thinking → tool → thinking → tool.
...
- **The DigitalOcean tier is GONE (2026-07-27)** — its BYOK billing account stopped being provided, so it was deleted rather than left failing on every walk. Do NOT re-add a tier without a live account behind it (a user's own OpenRouter key is a separate, still-supported BYOK provider).
...
- **A model whose deployment PINS a sampling parameter needs it sent** (`REQUIRED_TOP_P`, `packages/ai/src/agent.ts`): a deployment can 400 unless that exact value is on the wire, and the SDK sends none by default — silently killing a whole rung, visible only as a 400 partway down a walk. Empty today; add a slug if a provider ever rejects an omitted parameter.
...
- **`BYOK_ENCRYPTION_KEY` (min 32 chars) gates the whole feature** (and Sign in with ChatGPT). Unset = no App Home section, no per-user routing — secrets are never stored in the clear. Stretched with scrypt into an AES-256-GCM key (`lib/byok/crypto.ts`). **Changing it makes every stored secret unreadable**; users re-add.
...
Every failure mode above is readable from `journalctl -u kyto.service`. Lifecycle lines in order: `[agent] turn started` → `routed turn` → `attempt started` → `[stream] provider error mid-stream` → `[stream] attempt stream ended` (the `StreamTally`: textChars, toolCalls, finishReasons, errors) → `attempt handled the turn` / `attempt failed, falling back` → `turn complete` / `turn failed` (durationMs + `failedAttempts` = the whole walk). `streamAttempt`'s `onError` must stay — without it the SDK dumps a raw unattributed stack. `errorStatus()`/`deepErrorText()` dig the status/body out of the wrapped `AI_RetryError`/`APICallError` chain.
...
- One password stands between the public internet and a privilege grant: constant-time compare, **global lockout after 8 failures** (one legitimate user, so locking globally costs nothing and there's no per-IP bookkeeping to spoof), in-memory sessions (12h), `HttpOnly; Secure; SameSite=Strict; Path=/_dashboard` cookie, **per-session CSRF token on every mutation**.
...
- **Approving a GitHub request grants trust and stops there** — it does NOT replay the command (composed by a model in a thread that has since moved on; re-running it blind turns an approval click into an action nobody reviewed). The person asks kyto again.

## Manifest sync

`bun run sync:manifest` (apps/bot) pushes `slack-manifest.json` via `apps.manifest.update`. Needs a Slack **app configuration token** (not the bot/user token): `SLACK_APP_ID`, `SLACK_CONFIG_ACCESS_TOKEN`, optional `SLACK_CONFIG_REFRESH_TOKEN`. Scopes are declared in `slack-manifest.json` — update it when a tool needs a new one; scope changes require reinstalling the app.

## Host / deployment

- **Runs on the `oracle` server** (Oracle Linux 9, aarch64), migrated from `nest` after that account was suspended for an AUP violation (unrelated to kyto). Postgres is **local with no TLS**, so `packages/db/src/client.ts` uses `ssl: false` — keep it `false` while oracle is the deploy target.
- **`gh` is NOT in the Oracle Linux repos**; install the GitHub CLI from its official RPM repo, or authenticate git pushes with a PAT credential instead of `gh`.
...
**`main` is the branch actually deployed** (`kyto.service` tracks what's checked out here). `rebuild-on-upstream` is an old Pi-era branch; **`main` is a strict superset of it** (audited 2026-07-09). Its `MAX_RECURRING_RUNS = 20` global auto-cancel was deliberately NOT ported (would silently kill existing "forever" reminders).

## Database notes

New tables/columns are pushed with one-off SQL — `drizzle-kit push` prompts interactively (a rename decision) and hangs in a non-TTY shell. Use `ALTER TABLE … ADD COLUMN IF NOT EXISTS` / `CREATE TABLE IF NOT EXISTS`. `db:generate`/`db:push` work for a human at the CLI. `authorization` is a reserved word — quote it in DDL. `sandbox_sessions` is **orphaned scaffolding**; `thread_sandboxes` is the live one.
