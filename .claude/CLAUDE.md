...
- **NO GitHub credential enters a sandbox, and the write gate is at the HTTP layer** (`lib/github-proxy/`, mounted on the sites server; owner's call 2026-07-29, built 2026-08-06). The old design brokered the real PAT in an E2B egress rule that stapled `Authorization` onto every github.com request, so ANY process in the box — sshx, tmate, a shell script, `sh -c 'g''h …'` — was already `kyto-agent`, while `guardGithubCommand` only ever saw strings that came through a kyto TOOL. Now `gh` (via `GH_HOST`/`GH_ENTERPRISE_TOKEN`, which makes it speak to `/api/v3` + `/api/graphql` as if kyto were GHES) and `git` (via a global `insteadOf` + a credential helper that reads the per-command env) talk to the host, which classifies each request, runs the SAME `guardGithubTargets`, attaches the PAT itself, and claims on 2xx. A bare `curl api.github.com` in the sandbox is anonymous. Detail in TOOLS.md; do NOT reintroduce a `network` rule carrying the token.
...
- **Command prefix — `@kyto?focusmode @person`** (`lib/commands.ts`, owner's ask 2026-08-05). A message whose body (after leading mentions) starts `?word` or `!word` is answered by the HARNESS: `runCommandOrTurn` calls `handleCommand` first and returns the moment it says true, so a command **costs no model turn and never touches an in-flight turn's controller** — `stop` is the only one that deliberately reaches into a running turn. `?focusmode @a @b` / bare (= focus on me) / `?focusmode off`; same `thread.setFocus` state as the tool, so the owner stays exempt. An UNKNOWN `?word` is not a command and falls through to a normal turn — plenty of real messages open with a question mark. A non-focused user can't clear a focus (they're filtered in `bot.ts` before this runs); that is the point of focus, not a bug.
...
- **An ATTEMPT is "handled" iff IT produced reply text or a deliberate `skip`** (per-attempt, not per-turn). A model that ran tools but wrote nothing gets ONE `synthesizeFinalAnswer` nudge (same model, tools still ON so it can finish the work, told not to repeat a side effect that already happened) before falling back. `NO_TOOLS_NOTICE`/`tools: {}` is now only for `continueTruncatedReply`, where the work is done and only the prose needs finishing — the nudge path used that framing until 2026-08-05 and the model narrated "my tools are switched off" straight into the user's reply.
...
- **Prompt caching** (1h TTL) + **`maxOutputTokens: 8000`** on the metered proxies defuse HackClub's pessimistic spend projection. **Nothing volatile may enter the system prompt** — it is one string, so one changed byte throws away the whole system+tools prefix; the per-turn clock and message id live in the user message's volatile tail. `cache-probe.ts` logs any step whose prompt is not a pure append.
