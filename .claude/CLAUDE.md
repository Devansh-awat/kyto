...
- **A `skip` ENDS the attempt** (`SKIP_TOOL_NAME` + `hasToolCall` in `streamAttempt`'s `stopWhen`): its tool result used to feed back into the loop, so a model that declined to answer was asked again what to do about the same message — 5+ Thinking→skip→Thinking cycles, budget spent on a message already ignored. The stop lives in `streamAttempt`, not per call site, so a new caller can't re-open it; the toolset registers the tool under that exact constant.
