...
- **It posts NO message of its own.** Its whole run renders inside the PARENT turn's plan block, pushed through the turn's `ChunkRelay` (`lib/agent/relay.ts`): a **Prompt** card, a **Model** card per attempt, the same interleaved thinking/tool cards a real turn shows (shared `renderStream`, no `emitText`), and a **Response** card holding the full report. Card ids are namespaced `subagent-N-…` and titles prefixed with the label (`resolveIdentity('subagent')` + optional `name`) — without the namespace a subagent's `reasoning-0`/`model-0` would land on top of the parent's. It used to post its own streamed "kyto subagent" message; that made kyto look like it answered twice (three times, once a wake turn restated it), so **the run is visible, the talking is the main agent's job**. Do NOT reintroduce a separate message.
- **`ChunkRelay` is why this works at all**: a subagent runs inside a tool `execute`, and while a FOREGROUND one runs the parent generator is blocked on that very tool call, so it cannot yield. `streamSegmented`'s `nextRendered` races the relay against the model stream — keeping the unconsumed `source.next()` promise when the relay wins, or a chunk would be dropped. Buffered, so a push that lands mid-race is never lost. Closed at turn end, so a background subagent outliving the turn stops pushing into a plan nobody drains.
