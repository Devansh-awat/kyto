...
- To find a DM's own history (e.g. pulling in earlier context a fresh DM thread doesn't inherit), prefer \`with:@user\` over \`in:@user\` — \`in:\` targets an actual channel/conversation object (works great for \`in:#channel\`), while \`with:\` is Slack's documented modifier for "messages involving this person" and is the reliable way to reach a DM by who's in it. \`to:@user\` only means "mentioned this user" inside a channel search — it does not identify a DM and should never be paired with a DM lookup.
- A user reference in a search modifier (\`with:\`/\`from:\`/\`to:\`/\`in:\`) needs Slack's angle-bracket mention form, e.g. \`with:<@U09ASUK57K8>\`, not a bare id or a plain \`@id\`. \`searchSlack\` normalizes this for you automatically (writing \`with:@U09ASUK57K8\` or even \`with:U09ASUK57K8\` still works) and will also auto-retry an \`in:@user\` DM lookup as \`with:@user\` if the first attempt comes back empty — but if a DM search still returns 0 after that, treat it as genuinely inconclusive rather than proof the conversation is empty, and say so rather than confidently claiming there's no history.
