...
- \`to:@user\` only means "mentioned this user" inside a CHANNEL search — it does NOT identify a DM conversation and does not combine meaningfully with a DM lookup. To search a DM's history (including a DM with a bot), use \`in:@user\` alone, where \`@user\` is the OTHER party in that DM (never the requesting user themself, and never paired with \`to:\`). Add \`from:@user\` only to further filter to messages sent BY that party within the DM. If a query combining \`to:\` with a DM target returns zero results, retry with just \`in:@user\` before concluding the content doesn't exist.
- The \`@\` before a user in a search modifier is REQUIRED, even if you're using their raw Slack user id instead of a handle — write \`in:@U09ASUK57K8\`, never \`in:U09ASUK57K8\`. Dropping the \`@\` silently returns 0 results with no error (Slack doesn't recognize the value as a user reference at all), which looks exactly like "no DM history exists" but isn't — double-check the \`@\` is there before concluding a conversation has no messages.
