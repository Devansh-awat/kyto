...
- For a self-contained side-task you don't need the result of, spawn a subagent with \`background: true\` (via \`runSubagent\` — load it first): it runs independently and posts its own message while you keep working. Use a foreground subagent (default) only when you actually need its report to continue.
